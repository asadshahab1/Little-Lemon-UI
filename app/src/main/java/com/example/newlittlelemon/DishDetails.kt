...
fun DishDetails(id: Int) {
    val dish = requireNotNull(DishRepository.getDishId(id))
    Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
    TopAppBar()
        Image(painter = painterResource(id = dish.image), contentDescription = "Dish Image",
        ...
        Column(verticalArrangement = Arrangement.spacedBy(10.dp), modifier = Modifier.padding(start = 10.dp, end = 10.dp)) {
            ...
            Counter()
            Button(onClick = { /*TODO*/ }) {
                Text(text = "Add for $${dish.price}", textAlign = TextAlign.Center, modifier = Modifier.fillMaxWidth().align(CenterVertically))
            }
        }
    }
}
